...
func (s *Service) IngestPreparedChunks(ctx context.Context, chunks []PreparedChunk) (int, error) {
	...
	for _, ch := range chunks {
		...
		if uint64(len(vec)) != s.vectorSize {
			return 0, fmt.Errorf("向量维度=%d 与 RAG_VECTOR_SIZE=%d 不一致", len(vec), s.vectorSize)
		}
		kind := ch.Kind
		if kind == "" {
			kind = "doc"
		...
		id := chunkPointID(ch.Source, ch.Text)
		points = append(points, &qdrant.PointStruct{
			Id:      qdrant.NewIDNum(id),
			Vectors: qdrant.NewVectorsDense(vec),
			Payload: pl,
...
func chunkPointID(source, text string) uint64 {
	h := fnv.New64a()
...
func (s *Service) Search(ctx context.Context, query string, limit uint64) ([]Hit, error) {
	...
	for _, sp := range res {
		...
		if sp.Payload != nil {
			...
			if v := sp.Payload["source"]; v != nil {
				src = v.GetStringValue()
			}
			if v := sp.Payload["kind"]; v != nil {
				kind = v.GetStringValue()
			}
		}
		out = append(out, Hit{Score: sp.Score, Text: txt, Source: src, Kind: kind})
	}
	return out, nil
}
